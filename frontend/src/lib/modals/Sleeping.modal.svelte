...
<div class="flex flex-col gap-4 justify-center items-center h-full">
	{#if !$sleepingTimerStore}
		<button
			class="bg-primary text-gray-100 py-2 rounded-md hoverable w-full"
			on:click={() => sleep(30)}
		>
			Sleep in 30 minutes
		</button>
		...
		<div class="flex w-full gap-4">
			...
			<button
				class="bg-primary text-gray-100 p-2 rounded-md hoverable flex-grow"
	...
	{:else}
		<button
			class="bg-primary text-gray-100 py-4 px-8 rounded-md hoverable"
			on:click={() => clear()}
		>
			Clear sleep timer
		</button>
